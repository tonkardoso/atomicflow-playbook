...
**Ponytail during Execute (mandatory):** before writing code, load and follow the `ponytail` skill (`.cursor/skills/ponytail/SKILL.md`) at intensity **full** unless the user sets lite/ultra. Every task climbs the ladder (YAGNI → reuse → stdlib → native → installed dep → one-liner → minimum code). See [implement.md](references/implement.md) § "Ponytail during Execute".
