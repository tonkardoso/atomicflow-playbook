...
## MANDATORY: Ponytail during Execute

**Before writing any code in Execute**, read and follow the skill
`ponytail` (`.cursor/skills/ponytail/SKILL.md`) at intensity **full**
(unless the user asked for lite/ultra).

Every task implementation MUST climb the ponytail ladder before coding:
YAGNI → reuse in codebase → stdlib → native platform → installed dependency
→ one-liner → minimum working code. No speculative abstractions, no new
dependencies if avoidable, shortest correct diff.

If a deliberate shortcut has a known ceiling, mark it with a `ponytail:`
